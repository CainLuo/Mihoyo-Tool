...
- [x] 1. 创建 Domain 和 Path 枚举文件
  - [x] 1.1 新建 `core/src/main/ets/network/MihoyoDomain.ets`，定义 7 个 Host 枚举值（`TAKUMI_RECORD`、`TAKUMI`、`BBS`、`BBS_APIHUB`、`ACT_TAKUMI`、`PASSPORT`、`PASSPORT_V4`）
  - [x] 1.2 新建 `core/src/main/ets/network/MihoyoAccountApiPath.ets`，定义 5 个 Path 枚举值（含 `CREATE_VERIFICATION`、`VERIFY_VERIFICATION`）
  - [x] 1.3 新建 `core/src/main/ets/network/PassportApiPath.ets`，定义 `GET_COOKIE_TOKEN_BY_STOKEN`、`GET_LTOKEN_BY_STOKEN`、`VERIFY_LTOKEN` 三个枚举值
  - [x] 1.4 新建 `core/src/main/ets/network/GenshinApiPath.ets`，定义 4 个 Path 枚举值
  - [x] 1.5 新建 `core/src/main/ets/network/StarRailApiPath.ets`，定义 4 个 Path 枚举值
  - [x] 1.6 新建 `core/src/main/ets/network/ZZZApiPath.ets`，定义 4 个 Path 枚举值
  - [x] 1.7 新建 `core/src/main/ets/network/SignApiPath.ets`，定义大别野签到和各游戏签到的 Path 枚举值

- [x] 2. 创建 MihoyoApiService 抽象基类
  - [x] 2.1 新建 `core/src/main/ets/network/MihoyoApiService.ets`，定义 `abstract get()` 和 `abstract post()` 方法（`post` 的 `data` 参数类型为 `string | RequestParams`）
...
- [x] 4. 创建 ApiConfig
  - [x] 4.1 新建 `core/src/main/ets/network/ApiConfig.ets`，定义 `APP_VERSION`、`USER_AGENT`、`TOOL_VERSION_GENSHIN`、`TOOL_VERSION_STARRAIL` 常量
...
- [x] 5. 创建 DSUtilV2
  - [x] 5.1 新建 `core/src/main/ets/network/DSUtilV2.ets`，实现 `generateV1(params?, body?)`、`generateV2(params?, body?)`、`generateX6(params?, body?)` 三个静态方法
...
- [x] 6. 创建 MihoyoHeaderBuilder
  - [x] 6.1 新建 `core/src/main/ets/network/MihoyoHeaderBuilder.ets`，定义 `HeaderProfile` 枚举（`GAME_RECORD`、`ZZZ_GAME_RECORD`、`CALCULATE`、`BBS`、`SIGN`）
...
- [x] 7. 创建 MihoyoEnvironment 和 MihoyoApiServiceFactory
  - [x] 7.1 新建 `core/src/main/ets/network/MihoyoEnvironment.ets`，定义 `MOCK` 和 `RELEASE` 两个枚举值
  - [x] 7.2 新建 `core/src/main/ets/network/MihoyoApiServiceFactory.ets`，实现 `createAccountService`、`createGenshinService`、`createStarRailService`、`createZZZService`、`createSignService` 五个静态工厂方法
  - [x] 7.3 Mock 环境下 `context` 为必填参数，缺失时抛出 `Error`（禁止 `!` 非空断言）

- [x] 8. 创建 MihoyoAccountApiService
  - [x] 8.1 新建 `core/src/main/ets/network/MihoyoAccountApiService.ets`，继承 `MihoyoApiService`
...
- [x] 9. 创建 GenshinApiService
  - [x] 9.1 新建 `core/src/main/ets/network/GenshinApiService.ets`，继承 `MihoyoApiService`
...
- [x] 10. 创建 StarRailApiService
  - [x] 10.1 新建 `core/src/main/ets/network/StarRailApiService.ets`，继承 `MihoyoApiService`
...
- [x] 11. 创建 ZZZApiService
  - [x] 11.1 新建 `core/src/main/ets/network/ZZZApiService.ets`，继承 `MihoyoApiService`
...
- [x] 12. 创建 SignApiService
  - [x] 12.1 新建 `core/src/main/ets/network/SignApiService.ets`，继承 `MihoyoApiService`
...
- [x] 13. 更新 core/Index.ets 导出
  - [x] 13.1 在 `core/Index.ets` 中新增 `network/` 目录下所有公共类和枚举的导出（`MihoyoDomain`、各 ApiPath 枚举、`MihoyoApiService`、`MihoyoHeaderBuilder`、`HeaderProfile`、`ApiConfig`、`DSUtilV2`、`MihoyoEnvironment`、`MihoyoApiServiceFactory`、各 Service 类）
