...
- [x] 1. 创建 RdbManager 和基础表
  - [x] 1.1 新建 `core/src/main/ets/database/RdbManager.ets`，实现 `init(context)` 方法，使用 `SecurityLevel.S2` 打开数据库，执行 `PRAGMA foreign_keys = ON`
...
- [x] 2. 创建原神专用表（DDL）
  - [x] 2.1 在 `RdbManager.createTables()` 中添加 `genshin_character_list` 建表语句（含唯一索引和查询索引）
...
- [x] 6. 创建 BBSDao
  - [x] 6.1 新建 `core/src/main/ets/database/BBSDao.ets`，实现 `AccountDao` 类：`upsert(row: AccountRow)`、`findAll()`、`findById(id)`、`deleteById(id)`
  - [x] 6.2 实现 `GameRoleDao` 类：`upsertAll(rows: GameRoleRow[])`、`findByAccountId(accountId)`、`deleteByAccountId(accountId)`

- [x] 7. 创建 GenshinDao
  - [x] 7.1 新建 `core/src/main/ets/database/GenshinDao.ets`，实现 `GenshinCharacterListDao`：`upsertAll(rows)`、`findAll(accountId, roleUid)`、`deleteAll(accountId, roleUid)`
...
- [x] 8. 创建 StarRailDao
  - [x] 8.1 新建 `core/src/main/ets/database/StarRailDao.ets`，实现 `StarRailAvatarBasicDao`：`upsertAll(rows)`、`findAll(accountId, roleUid)`、`deleteAll(accountId, roleUid)`
...
- [x] 9. 创建 ZZZDao
  - [x] 9.1 新建 `core/src/main/ets/database/ZZZDao.ets`，实现 `ZZZAvatarBasicDao`：`upsertAll(rows)`、`findAll(accountId, roleUid)`、`deleteAll(accountId, roleUid)`
...
- [x] 10. 创建 SyncMetaDao
  - [x] 10.1 新建 `core/src/main/ets/database/SyncMetaDao.ets`，实现 `upsert(row)`、`findOne(accountId, roleUid, dataType)`、`updateStatus(accountId, roleUid, dataType, status, errorMsg?)`

- [x] 11. 创建数据行模型（Row 类）
  - [x] 11.1 新建 `core/src/main/ets/models/AccountRow.ets`，定义 `AccountRow` 类，字段对应 `account_table` 所有列
  - [x] 11.2 新建 `core/src/main/ets/models/GameRoleRow.ets`，定义 `GameRoleRow` 类，字段对应 `game_role_table` 所有列
  - [x] 11.3 新建 `core/src/main/ets/models/GenshinRows.ets`，定义 `GenshinCharacterListRow`、`GenshinDailyNoteRow`、`GenshinCharacterDetailRow`、`GenshinCharacterComputeRow` 四个类
  - [x] 11.4 新建 `core/src/main/ets/models/StarRailRows.ets`，定义 `StarRailAvatarBasicRow`、`StarRailDailyNoteRow`、`StarRailAvatarInfoRow`、`StarRailAvatarComputeRow` 四个类
  - [x] 11.5 新建 `core/src/main/ets/models/ZZZRows.ets`，定义 `ZZZAvatarBasicRow`、`ZZZDailyNoteRow`、`ZZZAvatarInfoRow`、`ZZZAvatarComputeRow` 四个类
  - [x] 11.6 新建 `core/src/main/ets/models/SyncMetaRow.ets`，定义 `SyncMetaRow` 类

- [x] 12. 创建 GameRepository 抽象基类
  - [x] 12.1 新建 `core/src/main/ets/repository/GameRepository.ets`，定义 `abstract class GameRepository`，包含 `abstract findAll()`、`abstract upsertAll()`、`abstract deleteAll()` 方法，以及 `handleApiErrorWithRefresh()` 公共方法

- [x] 13. 创建 BBSRepository
  - [x] 13.1 新建 `core/src/main/ets/repository/BBSRepository.ets`，实现账号登录、查询、删除方法
...
- [x] 14. 创建 GenshinRepository
  - [x] 14.1 新建 `core/src/main/ets/repository/GenshinRepository.ets`，继承 `GameRepository`
...
- [x] 15. 创建 StarRailRepository
  - [x] 15.1 新建 `core/src/main/ets/repository/StarRailRepository.ets`，继承 `GameRepository`
  - [x] 15.2 实现 `getDailyNote`、`getAvatarBasic`、`getAvatarInfo`、`compute` 四个方法，各自写入对应专用表并更新 `sync_meta`

- [x] 16. 创建 ZZZRepository
  - [x] 16.1 新建 `core/src/main/ets/repository/ZZZRepository.ets`，继承 `GameRepository`
  - [x] 16.2 实现 `getDailyNote`、`getAvatarBasic`、`getAvatarInfo`、`compute` 四个方法，各自写入对应专用表并更新 `sync_meta`

- [x] 17. 创建 SignRepository
  - [x] 17.1 新建 `core/src/main/ets/repository/SignRepository.ets`，持有 `SignApiService` 实例
...
- [x] 18. 创建 CoreInitializer
  - [x] 18.1 新建 `core/src/main/ets/CoreInitializer.ets`，实现 `initCore({ isMock, context })` 方法
  - [x] 18.2 初始化流程：`RdbManager.init()` → `ApiConfig.preloadDeviceId/Fp()` → `MihoyoApiServiceFactory.create*()` → 各 Repository `setService()`
  - [x] 18.3 根据 `isMock` 参数决定使用 Mock Service 还是 Release Service

- [x] 19. 更新 core/Index.ets 导出
  - [x] 19.1 在 `core/Index.ets` 中新增所有 v2 目录下的类和枚举的导出（`CoreInitializer`、各 Repository、各 Row 类、`SyncDataType`、`ApiErrors` 等）
...
- [x] 20. 将 entry 模块切换到新 Repository
  ...
  - [x] 20.2 将 `EntryAbility.onCreate` 中的初始化调用从 `CoreInitializer` 切换到 `CoreInitializer`
