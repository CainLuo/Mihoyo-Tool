#!/bin/bash
# 构建 release product（真实 API Service）并安装到模拟器
# 支持通过 Proxyman Local Rewrite 拦截流量
#
# 用法：
#   bash run-release.sh                    # 直接连真实 API
#   bash run-release.sh --proxy 192.168.1.x:9090   # 流量走 Proxyman 代理
#   bash run-release.sh --clear-proxy      # 清除模拟器代理设置
#
# Proxyman 使用步骤：
#   1. Proxyman → Tools → Local Rewrite，添加规则拦截目标 URL
#   2. 查看 Mac IP：System Settings → Wi-Fi → Details → IP Address
#   3. bash run-release.sh --proxy <Mac IP>:9090
...
if [ -n "$PROXY_HOST" ]; then
  echo "✅ 完成（release 模式，流量经过 Proxyman: $PROXY_HOST）"
  echo "   清除代理：bash run-release.sh --clear-proxy"
else
  echo "✅ 完成（release 模式，直连真实 API）"
fi
